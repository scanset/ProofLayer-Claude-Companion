...
**Binds to:** `M365::User` — depth-0 (N=1) when bound directly to the asset; fans out via the `link` relation when bound to a container.

---

## Injection / Projection

The `m365_user_auth_methods_scoped` projection fills the injected `OBJECT` fields from the bound asset's metadata:

| OBJECT field | Source | Required |
| ------------ | ------ | -------- |
| `path` | literal `users` | **Yes** |
| `resource_id` | `metadata.provider_id` | **Yes** |
| `sub_path` | literal `authentication/methods` | No |

`target_asset_type`: `M365::User`. No hardcoded resource id appears in the policy — the binding determines scope.
