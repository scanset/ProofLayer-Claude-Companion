# m365_conditional_access_policy_scoped
...
**Binds to:** `M365::ConditionalAccessPolicy` — depth-0 (N=1) when bound directly to the asset; fans out via the `link` relation when bound to a container.

---

## Injection / Projection

The `m365_conditional_access_policy_scoped` projection fills the injected `OBJECT` fields from the bound asset's metadata:

| OBJECT field | Source | Required |
| ------------ | ------ | -------- |
| `path` | literal `identity/conditionalAccess/policies` | **Yes** |
| `resource_id` | `metadata.provider_id` | **Yes** |

`target_asset_type`: `M365::ConditionalAccessPolicy`. No hardcoded resource id appears in the policy — the binding determines scope.

**Placeholder (in the policy `DEF`):**

```esp
SET conditionalaccesspolicys union
    OBJECT t
        target `M365::ConditionalAccessPolicy`
        link `contains`
