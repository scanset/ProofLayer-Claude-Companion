...
Single-CTN discovery primitive: issues a `GET` against an arbitrary
Microsoft Graph collection path (`https://graph.microsoft.com/v1.0/<path>`)
and returns the result rows as a flat array. One CTN covers every Graph
collection — users, groups, devices, conditional access policies, Intune
managed devices, Purview policies, SharePoint sites, Teams. New
resources are added at the discovery layer (one row per resource type
in `inventory::discover_m365::GRAPH_PATHS`), not by authoring a new
CTN per type.

**Platform:** Microsoft Graph (requires an Entra ID app registration
with read permissions for the target resource type and admin consent
granted)
**Collection method:** HTTPS `GET` with bearer token, automatic
pagination via `@odata.nextLink`

**Note:** A multi-page collection (e.g. /users on a tenant with several
hundred users) is typically 1-3s total. The collector caps results at
100,000 rows per invocation as a safety net.
