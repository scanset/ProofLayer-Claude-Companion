...
**Binds to:** `M365::SharePointSite` — depth-0 (N=1) when bound directly to the asset; fans out via the `link` relation when bound to a container.

---

## Injection / Projection

The `m365_sharepoint_site_scoped` projection fills the injected `OBJECT` fields from the bound asset's metadata:

| OBJECT field | Source | Required |
| ------------ | ------ | -------- |
| `path` | literal `sites` | **Yes** |
| `resource_id` | `metadata.provider_id` | **Yes** |

`target_asset_type`: `M365::SharePointSite`. No hardcoded resource id appears in the policy — the binding determines scope.

**Placeholder (in the policy `DEF`):**

```esp
SET sharepointsites union
    OBJECT t
        target `M365::SharePointSite`
        link `contains`
