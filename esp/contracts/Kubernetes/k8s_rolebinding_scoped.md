# k8s_rolebinding_scoped

## Overview

Per-asset, scoped-injection variant of `k8s_api_query` bound to a single
namespaced `K8s::RoleBinding`. At dispatch the scoped-injection engine fills
the OBJECT's `path` from the bound asset's `api_path` metadata, so the
contract re-queries exactly one live binding and the verdict reflects the
current subject→role grant in that namespace.
...
**Target asset type:** `K8s::RoleBinding`
**RECORD:** yes
...
|-----------------|--------|----------|-------------|
| `path` | `SelfMetadata("api_path")` | **Yes** | `/apis/rbac.authorization.k8s.io/v1/namespaces/<ns>/rolebindings/<name>` |

The single-resource GET returns one object, wrapped into a one-element `rows`
array — record paths root at `*.`. A RoleBinding's `roleRef` may target a Role
(namespaced) or a ClusterRole (cluster-scoped, scoped into this namespace).
...
```
GET https://<apiserver>/apis/rbac.authorization.k8s.io/v1/namespaces/<ns>/rolebindings/<name>
...
{
  "kind": "RoleBinding",
  "metadata": { "name": "read-config", "namespace": "app" },
...
| Path | Type | Example |
|------|------|---------|
| `*.metadata.name` | string | `"read-config"` |
| `*.metadata.namespace` | string | `"app"` |
| `*.roleRef.kind` | string | `"Role"`, `"ClusterRole"` |
| `*.roleRef.name` | string | `"configmap-reader"` |
| `*.subjects.*.kind` | string | `"ServiceAccount"`, `"User"`, `"Group"` |
| `*.subjects.*.name` | string | `"default"`, `"system:masters"` |
...
## ESP Example (avoid system:masters in a namespace binding)

```esp
SET bindings union
    OBJECT t
        target `K8s::RoleBinding`
        link `contains`
...
| CTN | Relationship |
|-----|--------------|
| `k8s_role_scoped` | The Role referenced by `roleRef` (when `kind == Role`) |
| `k8s_clusterrole_scoped` | The ClusterRole referenced by `roleRef` (when `kind == ClusterRole`) |
| `k8s_serviceaccount_scoped` | A `ServiceAccount` subject of this binding |
| `k8s_api_query` | Base CTN; cluster-wide bulk inventory of all RoleBindings |
