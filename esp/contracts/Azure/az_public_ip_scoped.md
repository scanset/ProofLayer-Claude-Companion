...
**Injection-native variant of [`az_public_ip`](az_public_ip.md).** Same checking logic
— it reuses the `az_public_ip` check logic **verbatim** (no edits to the base) — and the same
collected-data and state surface. The only difference is *how policies bind to
it*: `az_public_ip_scoped` is the target of dispatch-time **template injection**.

You bind a policy to a **resource group**, a **subscription**, or a **single
Public IP**, and at scan time the dispatcher walks the asset graph to the
`Microsoft.Network/publicIPAddresses` descendants of the bound asset and fills one OBJECT per resource
...
`az_public_ip_scoped` declares a dispatch-time projection
...
| Property | Value |
| --- | --- |
| `ctn_name` | `az_public_ip_scoped` |
| `target_asset_type` | `Microsoft.Network/publicIPAddresses` |
| `name` | `<- display_name` |
...
Binding: subscription -> all Public IPs under it; resource group -> that RG's
Public IPs; a single Public IP -> itself (depth-0, N=1).
...
[`az_public_ip`](az_public_ip.md#object-fields).

## Commands Executed / Collected Data / State Fields

**Identical to [`az_public_ip`](az_public_ip.md)** — the check logic is reused
...
```esp
META
    esp_id `az_public_ip_scoped-injection-001`
    version `1.0.0`
    dsl_schema_version `1.0.0`
    platform `azure`
    criticality `medium`
    control_mapping `INTERNAL:SCOPED-INJECTION`
    title `Public IP (scoped injection)`
    target_asset_type `Microsoft.Network/publicIPAddresses`
META_END
...
`TEST all all` requires every resolved Public IP to satisfy the state — "X for
...
| --- | --- |
| [`az_public_ip`](az_public_ip.md) | The non-injection sibling whose check logic this reuses verbatim. |
