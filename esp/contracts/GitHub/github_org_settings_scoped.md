...
The `github_org_settings_scoped` projection fills the injected `OBJECT` fields from the bound asset's metadata:

| OBJECT field | Source | Required |
| ------------ | ------ | -------- |
| `login` | `metadata.login` | **Yes** |
...
DEF
    ...
    STATE outside_collab_restricted
        found boolean = true
        members_can_invite_outside_collaborators boolean = false
    STATE_END

    CRI AND
        CTN github_org_settings_scoped
            TEST all all AND
            STATE_REF outside_collab_restricted
            SET_REF github_orgs
        CTN_END
    CRI_END
DEF_END
