# aws_securityhub_account_scoped

## Overview

Scoped-injection variant of [`aws_securityhub_account`](aws_securityhub_account.md). The collector, command(s), collected data, and state fields are **identical to the base CTN** (reused verbatim) — this variant only changes *how the `OBJECT` is populated*: at dispatch the `inject_from_bound_asset` placeholder is replaced by one concrete `OBJECT` per resolved asset, filled from that asset's metadata. One policy file scans every matching resource, naming none.

**Binds to:** `AWS::SecurityHub::Hub` — depth-0 (N=1) when bound directly to the asset; fans out via the `link` relation when bound to a container.

---

## Injection / Projection

The `aws_securityhub_account_scoped` projection fills the injected `OBJECT` fields from the bound asset's metadata:

| OBJECT field | Source | Required |
| ------------ | ------ | -------- |
| `region` | `metadata.region` | No |

`target_asset_type`: `AWS::SecurityHub::Hub`. No hardcoded resource id appears in the policy — the binding determines scope.
...
DEF
    ...
    CRI AND
        ...
        CTN aws_securityhub_account_scoped
            TEST all all AND
            STATE_REF securityhub_evaluation_active
            SET_REF security_hub
        CTN_END
