...
Scoped-injection variant of [`aws_iam_role`](aws_iam_role.md). The collector, command(s), collected data, and state fields are **identical to the base CTN** (reused verbatim) — this variant only changes *how the `OBJECT` is populated*: at dispatch the `inject_from_bound_asset` placeholder is replaced by one concrete `OBJECT` per resolved asset, filled from that asset's metadata. One policy file scans every matching resource, naming none.

**Binds to:** `AWS::IAM::Role` — depth-0 (N=1) when bound directly to the asset; fans out via the `link` relation when bound to a container.

---

## Injection / Projection

The `aws_iam_role_scoped` projection fills the injected `OBJECT` fields from the bound asset's metadata:

| OBJECT field | Source | Required |
| ------------ | ------ | -------- |
| `role_name` | `metadata.role_name` | **Yes** |
| `region` | `metadata.region` | No |

`target_asset_type`: `AWS::IAM::Role`. No hardcoded resource id appears in the policy — the binding determines scope.
...
DEF
    ...
    CRI AND
        CTN aws_iam_role_scoped
            TEST all all AND
            STATE_REF backup_role_check
            SET_REF backup_role_set
        CTN_END
    CRI_END
DEF_END
```
