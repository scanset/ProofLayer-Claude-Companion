...
Scoped-injection variant of [`aws_flow_log`](aws_flow_log.md). The collector, command(s), collected data, and state fields are **identical to the base CTN** (reused verbatim) — this variant only changes *how the `OBJECT` is populated*: at dispatch the `inject_from_bound_asset` placeholder is replaced by one concrete `OBJECT` per resolved asset, filled from that asset's metadata. One policy file scans every matching resource, naming none.

**Binds to:** `AWS::EC2::VpcFlowLog` — depth-0 (N=1) when bound directly to the asset; fans out via the `link` relation when bound to a container.

---

## Injection / Projection

The `aws_flow_log_scoped` projection fills the injected `OBJECT` fields from the bound asset's metadata:

| OBJECT field | Source | Required |
| ------------ | ------ | -------- |
| `flow_log_id` | `metadata.flow_log_id` | **Yes** |
| `region` | `metadata.region` | No |

`target_asset_type`: `AWS::EC2::VpcFlowLog`. No hardcoded resource id appears in the policy — the binding determines scope.

**Placeholder (in the policy `DEF`):**

```esp
SET vpcflowlogs union
    OBJECT t
        target `AWS::EC2::VpcFlowLog`
        link `contains`
...
DEF
    ...
    CRI AND
        CTN aws_flow_log_scoped
            TEST all all AND
            STATE_REF vpc_flow_logs_check
            SET_REF vpc_flow_logs_set
        CTN_END
    CRI_END
DEF_END
```
