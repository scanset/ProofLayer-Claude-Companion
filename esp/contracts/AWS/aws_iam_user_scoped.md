...
Scoped-injection variant of [`aws_iam_user`](aws_iam_user.md). The collector, command(s), collected data, and state fields are **identical to the base CTN** (reused verbatim) — this variant only changes *how the `OBJECT` is populated*: at dispatch the `inject_from_bound_asset` placeholder is replaced by one concrete `OBJECT` per resolved asset, filled from that asset's metadata. One policy file scans every matching resource, naming none.

**Binds to:** `AWS::IAM::User` — depth-0 (N=1) when bound directly to the asset; fans out via the `link` relation when bound to a container.

---

## Injection / Projection

The `aws_iam_user_scoped` projection fills the injected `OBJECT` fields from the bound asset's metadata:

| OBJECT field | Source | Required |
| ------------ | ------ | -------- |
| `user_name` | `metadata.user_name` | **Yes** |
| `region` | `metadata.region` | No |

`target_asset_type`: `AWS::IAM::User`. No hardcoded resource id appears in the policy — the binding determines scope.
...
DEF
    ...
    STATE esp_daemon_check
        found boolean = true
        access_key_count int <= 1
    STATE_END

    CRI AND
        CTN aws_iam_user_scoped
            TEST all all AND
            STATE_REF esp_daemon_check
            SET_REF esp_daemon_set
        CTN_END
    CRI_END
DEF_END
```
