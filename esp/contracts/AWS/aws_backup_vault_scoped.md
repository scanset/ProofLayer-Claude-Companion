# aws_backup_vault_scoped

## Overview

Scoped-injection variant of [`aws_backup_vault`](aws_backup_vault.md). The collector, command(s), collected data, and state fields are **identical to the base CTN** (reused verbatim) — this variant only changes *how the `OBJECT` is populated*: at dispatch the `inject_from_bound_asset` placeholder is replaced by one concrete `OBJECT` per resolved asset, filled from that asset's metadata. One policy file scans every matching resource, naming none.

**Binds to:** `AWS::Backup::BackupVault` — depth-0 (N=1) when bound directly to the asset; fans out via the `link` relation when bound to a container.

---

## Injection / Projection

The `aws_backup_vault_scoped` projection fills the injected `OBJECT` fields from the bound asset's metadata:

| OBJECT field | Source | Required |
| ------------ | ------ | -------- |
| `vault_name` | `metadata.vault_name` | **Yes** |
| `region` | `metadata.region` | No |

`target_asset_type`: `AWS::Backup::BackupVault`. No hardcoded resource id appears in the policy — the binding determines scope.

**Placeholder (in the policy `DEF`):**

```esp
SET backupvaults union
    OBJECT t
        target `AWS::Backup::BackupVault`
        link `contains`
...
DEF
    ...
    STATE backup_vault_check
        found boolean = true
        encryption_key_arn string contains `KMS_KEY_ID`
        locked boolean = true
    STATE_END

    CRI AND
        CTN aws_backup_vault_scoped
            TEST all all AND
            STATE_REF backup_vault_check
            SET_REF backup_vault_set
        CTN_END
    CRI_END
DEF_END
```
